...
export class StoryPictureComponent {
  ...
  @Output() pictureReset = new EventEmitter();

  @Output() rawFile = new EventEmitter<File>();

  constructor(private sanitizer: DomSanitizer) {}

  get pictureExists(): boolean {
    return this.parent.get('picture.slug')!.hasError('pictureExists');
  }

  patchFileInfo(file: File): void {
    const objectURL = this.sanitizer.bypassSecurityTrustUrl(
      window.URL.createObjectURL(file)
    );
    const fileName = file.name.split('.')[0].replace(/-/g, ' ');
    this.fileUrl = objectURL;
    this.parent.get('picture')!.patchValue({
      file: {
